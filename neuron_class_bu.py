...
class neuron(object):
    ...
    def __init__(self, datafile, rawdatadir, datatype='npy', cell_idx=0, start_day=0, end_day=1, silicon=False,
                 probenumber=1, fs=25000, file_list=[]):
        if datatype == 'npy':
            ...
            if len(file_list) > 0:
                ...
                if (silicon):
                    ...
                    spikefiles = [channelFiles[i] for i in range(len(channelFiles)) if
                                  channelFiles[i] in np.sort(glob.glob("*spike_times*.npy"))]
                    # print("spike_files: ", spikefiles)
                    clustfiles = [channelFiles[i] for i in range(len(channelFiles)) if
                                  channelFiles[i] in np.sort(glob.glob("*spike_clusters*.npy"))]
                    # sorts any peak channel files found in folder
                    peakfiles = [channelFiles[i] for i in range(len(channelFiles)) if (
                                channelFiles[i] in np.sort(glob.glob("*peakchannel*.npy")) or channelFiles[
                            i] in np.sort(glob.glob("*max_channel*.npy")))]
                    # sorts wavefiles in two forms, named "waveform" or "templates"
                    wavefiles = [channelFiles[i] for i in range(len(channelFiles)) if
                                 channelFiles[i] in np.sort(glob.glob("*waveform*.npy"))]
                    templates_all = [channelFiles[i] for i in range(len(channelFiles)) if
                                     channelFiles[i] in np.sort(glob.glob("*templates*.npy"))]
                    # since there are multiple files that have "templates" at the end this pulls out only the ones we want
                    templates_wf = [fn for fn in templates_all if
                                    fn not in glob.glob("*spike*.npy") and fn not in glob.glob(
                                        "*similar*.npy") and fn not in glob.glob(
                                        "*number_of_*.npy") and fn not in glob.glob("*templates_in_clust.npy")]
                    ...
                    aq = [channelFiles[i] for i in range(len(channelFiles)) if
                          channelFiles[i] in np.sort(glob.glob("*qual*.npy"))]
                    # looks for scrubbed quality and loads if possible
                    sq = [channelFiles[i] for i in range(len(channelFiles)) if
                          channelFiles[i] in np.sort(glob.glob("*scrubbed*.npy"))]
            ...
            clocktimeSIdx = spikefiles[0].find(
                "int16_") + 6
            ...
            print("\nData set information:\nPeak Channels: {}\nWaveform Template: {}\nNumber of clusters: {}".format(
                has_peak_files, has_twf, len(self.unique_clusters)))
            ...
            for idx, f in enumerate(sleepFiles):
                ...
                last = idx
            print('sleep states recorded through hour {}'.format(last))
            ...
            try:
                self.animal = np.array(f['neurons/neuron_' + str(cell_idx) + '/animal'][:],
                                       dtype=np.int8).tostring().decode("ascii")
            except:
            ...
            self.cont_stat = f['neurons/neuron_' + str(
                cell_idx) + '/cont_stat'].value  # bool(f['neurons/neuron_'+str(cell_idx)+'/cont_stat'][0])
    ...
    def spikeTimeToClockTime(self):
        ...
        for time in self.time:
            ...
            tnew = t + tdelt
            strTime = "{}-{}-{}___{}:{}:{}:{}".format(tnew.year, tnew.month, tnew.day, tnew.hour, tnew.minute,
                                                      tnew.second, tnew.microsecond)
        ...
        if self.datatype == 'hdf5':
            ...
            for p in lt_off / binsz:
                currentAxis.add_patch(
                    patches.Rectangle((p, 0), 12, ylim, facecolor="grey", alpha=0.1, edgecolor="none"))
